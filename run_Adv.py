...
from exp.exp_adversarial_AD import Exp_AdGenerator
import random
import numpy as np
...
# basic config
parser.add_argument('--mode', type=str, default='train', choices=['train', 'test','test_ori', 'gen_adv_test','gen_adv_train', 'test_adv','visualize_adv','eval_adv','train_adv_gen', 'defence_train', 'defence_test'])
...
parser.add_argument('--attack_method', type=str, default='FGSM', help='attack method')
parser.add_argument('--epsilon', type=float, default=0.5, help='noise ampitude of attack, 0.5 for training, others for test')
parser.add_argument('--adv_epoch', type=int, default=3, help='epoch of adversarial training')
parser.add_argument('--attack_target', type=str, default='Precision', choices=['Precision','Recall','Balanced'])

parser.add_argument('--apply_adv_aug', type=eval, default='False')
parser.add_argument('--cross_attack', type=eval, default='False')
...
if __name__ == '__main__':
    ...
    if args.mode=='test':
        exp.test()
    elif args.mode=='test_ori':
        exp.test_ori()
    elif args.mode == 'gen_adv_test':
        exp.gen_adv_test()
    elif args.mode == 'test_adv':
        exp.test_adversarial()
    elif args.mode == 'visualize_adv':
        exp.visualize_adv()
    elif args.mode == 'eval_adv':
        exp.evaluation_adv()
    elif args.mode == 'train_adv_gen':
        exp.train_adv_gen()
    elif args.mode == 'gen_adv_train':
        exp.gen_adv_train()
    elif args.mode == 'defence_train':
        exp.defence_train()
    elif args.mode == 'defence_test':
        exp.defence_test()
